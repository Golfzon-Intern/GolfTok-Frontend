...
export function getCommentLike(targetId) {
  return http.get('/like/comment', { params: { commentId: targetId } });
...
export function addCommentLike(targetId) {
  return http.post('/like/comment', { commentId: targetId });
}

// 특정 댓글 좋아요 삭제
export function deleteCommentLiked(targetId) {
  return http.delete('/like/comment', { params: { commentId: targetId } });
}
